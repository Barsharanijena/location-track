...
function bearing(a: Coordinates, b: Coordinates): number {
  ...
  const x =
    ...
    Math.sin((a.lat * Math.PI) / 180) * Math.cos((b.lat * Math.PI) / 180) * Math.cos((dLng * Math.PI) / 180);
...
function haversineKm(a: Coordinates, b: Coordinates): number {
  ...
  const cos = Math.cos((a.lat * Math.PI) / 180) * Math.cos((b.lat * Math.PI) / 180);
  const c = 2 * Math.atan2(
    Math.sqrt(sinDLat * sinDLat + cos * sinDLng * sinDLng),
    Math.sqrt(1 - sinDLat * sinDLat - cos * sinDLng * sinDLng)
  );
  return R * c;
}
...
export function startSimulation(
  ...
  const timer = setInterval(() => {
    ...
    if (progress >= 1) { segmentIndex += 1; progress = 0; }

    if (segmentIndex >= route.length - 1) {
      onUpdate({ agentId, agentName, coordinates: route[route.length - 1], speed: 0, heading: 0, timestamp: Date.now(), status: "arrived" });
      stopSimulation(agentId);
      onComplete(agentId);
      return;
    }

    const cur = route[segmentIndex];
    const next = route[segmentIndex + 1];
    onUpdate({ agentId, agentName, coordinates: interpolate(cur, next, Math.min(progress, 1)), speed: speedKmh, heading: bearing(cur, next), timestamp: Date.now(), status: "en_route" });
  }, intervalMs);
...
export function stopSimulation(agentId: string): void {
  ...
  if (sim) { clearInterval(sim.timer); activeSimulations.delete(agentId); }
}
